...
export default function SubscriptionHistoryContent() {
  ...
  const filtered = history.filter(entry =>
    ...
    entry.subscription.toLowerCase().includes(searchTerm.toLowerCase()) ||
    entry.status.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const displayHistory = showNoData ? [] : filtered;
        ...
        <table className="w-full border-collapse rounded-lg overflow-hidden">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="text-left py-3 px-4 text-xs font-semibold text-gray-700 uppercase tracking-wider">Subscription</th>
            ...
            {displayHistory.length === 0 ? (
              ...
              displayHistory.map((entry, index) => (
                <tr key={entry.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="py-3 px-4 text-sm text-gray-700 font-medium">{entry.name}</td>
                  <td className="py-3 px-4 text-sm text-gray-700">{entry.subscription}</td>
      ...
      <div className="flex justify-between items-center mt-6 text-sm text-gray-600">
        ...
        <div>
          {displayHistory.length === 0
            ...
            : `Showing 1 to ${displayHistory.length} of ${displayHistory.length} entries`}
        </div>

        <div className="flex items-center gap-1">
          <button className="px-3 py-1 border border-gray-300 rounded hover:bg-gray-50 transition-colors disabled:opacity-50" disabled={displayHistory.length === 0}>
            Previous
          </button>
          <button className={`px-3 py-1 rounded ${displayHistory.length === 0 ? 'bg-gray-300 text-gray-500' : 'bg-green-600 text-white'}`}>
            1
          </button>
          <button className="px-3 py-1 border border-gray-300 rounded hover:bg-gray-50 transition-colors disabled:opacity-50" disabled={displayHistory.length === 0}>
            Next
